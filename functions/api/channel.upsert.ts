...
export const onRequestPost: PagesFunction<{ KV: KVNamespace }> = async (
  ...
  const now = Date.now();
  const chHandle = normalizeHandle(body.handle || "");
  if (!isValidHandle(chHandle))
    return new Response("Bad handle", { status: 400 });
  const existingOwner = await kv.get(`channel-handle:${chHandle}`);
  const key = `channel:${userId}`;
  const current = await kvGetJson<Channel>(kv, key);
  if (existingOwner && existingOwner !== userId)
    return new Response("Channel handle taken", { status: 409 });
  ...
  await kv.put(`channel-handle:${chHandle}`, userId);
