...
const ok  = (b: any, init = 200) => NextResponse.json(b, typeof init === "number" ? { status: init } : init);
const bad = (m: string, s = 400) => ok({ error: m }, s);

function db() {
  const d1 = (getRequestContext().env as any).DB as D1Database;
  if (!d1) throw new Error("D1 binding not found (check wrangler.toml [[env.production.d1_databases]])");
  return d1;
}
...
export async function GET(req: Request) {
  try {
    ...
    let where = "1=1";
    if (onlyLive) { where += " AND s.is_live = 1"; }
    if (creator_id) { where += " AND s.creator_id = ?"; args.push(Number(creator_id)); }

    const q = await db()
      .prepare(`SELECT s.id, s.title, s.is_live, s.viewers, s.thumbnail_url, s.started_at,
