...
export async function GET(req:Request,context:any){try{const db=context.env.DB as D1Database;const url=new URL(req.url);const creatorId=await resolveId(url.searchParams,db);if(!creatorId)return NextResponse.json({ok:false,error:"missing query: pass ?creator_id=… or ?handle=…"},{status:400});const {results}=await db.prepare(`SELECT c.handle AS following_handle, c.display_name AS following_name, f.created_at
 ...
 WHERE f.follower_id=? ORDER BY f.created_at DESC LIMIT 100`).bind(creatorId).all();return NextResponse.json({ok:true,[name]:results??[]});}catch(e:any){console.error("API /following:",e);return NextResponse.json({ok:false,error:e?.message??String(e)},{status:500});}}