        ...
        <div className="relative z-10 mx-auto flex h-full w-full max-w-5xl flex-col px-4 py-6">
          ...
          <div className="mx-auto mb-4 flex items-center gap-2 rounded-full bg-white/10 p-1">
            {GROUPS.map(g => (
              <button
                ...
                className={\`px-4 py-2 rounded-full text-sm font-medium transition \${group === g.key ? "bg-[var(--laser-green)] text-black" : "text-white/80 hover:text-white"}\`}
              >
                {g.label}
              </button>
            ))}
          ...
          <div className="mb-5 flex w-full snap-x items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
            {METRICS[group].map(m => (
              <button
                ...
                className={\`whitespace-nowrap rounded-full border px-3 py-1.5 text-xs font-semibold transition \${metric === m.key ? "border-[var(--laser-green)] bg-[var(--laser-green)] text-black" : "border-white/15 bg-black/30 text-white/80 hover:text-white"}\`}
              >
                {m.label}
              </button>
            ))}
          ...
          <div className="relative w-full grow overflow-hidden rounded-2xl border border-white/10 bg-black shadow-none backdrop-blur">
            ...
            <ol className="h-full overflow-y-auto no-scrollbar p-2 pr-3">
              {list.map((u, i) => (
                <li key={u.id} className="group flex items-center gap-3 rounded-xl px-2 py-2 hover:bg-white/5">
                  ...
                  <div className="min-w-0 grow">
                    ...
                    <div className="text-xs text-white/50">{METRICS[group].find(m => m.key === metric)?.label}</div>
                  </div>
