...
export default function LiveRow() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      requestAnimationFrame(() => {
        ...
        const ts2 = performance.now(); const prevTs2 = (el as any)._lastTs ?? ts2; (el as any)._lastTs = ts2; const speed = Math.abs(rawDelta) / Math.max(1, now - prevTs); el.style.scrollSnapType = speed > 1 ? "none" : "x proximity";
  ...
  return (
    <section aria-label="Live now" className="relative mt-6">
      ...
      <ul
        ref={listRef}
        className="flex gap-3 overflow-x-auto px-6   p-3 scrollbar-hide"
      >
        {data.map((it, idx) => (
